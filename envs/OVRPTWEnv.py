...
class OVRPTWEnv:
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1):
        if problems is not None:
            ...
            depot_xy, node_xy, node_demand, service_time, tw_start, tw_end = self.get_random_problems(batch_size, self.problem_size, normalized=True)
        ...
        self.reset_state.prob_emb = torch.FloatTensor([1, 1, 0, 0, 1]).unsqueeze(0).to(self.device)  # bit vector for [C, O, B, L, TW]
    ...
    def step(self, selected):
        ...
        # Note: different from VRPTW, since no need to return to depot for OVRPTW
    ...
    def generate_dataset(self, num_samples, problem_size, path):
        ...
        print("Save OVRPTW dataset to {}".format(path))
    ...
    def get_random_problems(self, batch_size, problem_size, normalized=True):
        ...
        if problem_size == 20:
            ...
            demand_scaler = 1000
        elif problem_size == 3000:
            demand_scaler = 1500
        elif problem_size == 4000:
            demand_scaler = 2000
