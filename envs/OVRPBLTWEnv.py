...
class OVRPBLTWEnv:
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1):
        ...
        self.reset_state.prob_emb = torch.FloatTensor([1, 1, 1, 1, 1]).unsqueeze(0).to(self.device)  # bit vector for [C, O, B, L, TW]
    ...
    def step(self, selected):
        ...
        # Note: different from VRPL, since no need to return to depot for OVRPBLTW
    ...
    def generate_dataset(self, num_samples, problem_size, path):
        ...
        print("Save OVRPBLTW dataset to {}".format(path))
