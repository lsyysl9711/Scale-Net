...
class OVRPBEnv:
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1):
        if problems is not None:
            ...
            depot_xy, node_xy, node_demand, capacity = self.get_random_problems(batch_size, self.problem_size, normalized=True)
        ...
        self.reset_state.prob_emb = torch.FloatTensor([1, 1, 1, 0, 0]).unsqueeze(0).to(self.device)  # bit vector for [C, O, B, L, TW]
    ...
    def generate_dataset(self, num_samples, problem_size, path):
        ...
        print("Save OVRPB dataset to {}".format(path))
